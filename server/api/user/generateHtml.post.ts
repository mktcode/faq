...
export default defineEventHandler(async (event) => {
  ...
  const response = await openai.responses.parse({
    ...
    model: 'gpt-5',
    instructions: `Generiere HTML und CSS Code für eine Website-Sektion basierend auf der Anweisung des Benutzers. Du bekommst die globalen Website-Einstellungen als Orientierung. Die Anweisungen kommen von einem technisch wenig versierten Nutzer. Denke also genau darüber nach, was der Nutzer möchte. Wenn der Nutzer nach Funktionen fragt, die nicht mit HTML/CSS umgesetzt werden können, erkläre ihm dies kurz im Notizfeld.
