...
if (typeof route.query.target === 'string') {
  targetITSupport.value = route.query.target === 'it-support'
}
...
const usernameHelp = computed(() => {
  if (targetITSupport.value) {
    return 'Ihren Benutzernamen benötigen Sie zum Anmelden. Er ist außerdem Teil Ihrer vorläufigen Domain. Eine eigene Domain können Sie später verbinden, falls Sie die kostenlose Website nutzen möchten.'
  }
  return 'Ihren Benutzernamen benötigen Sie zum Anmelden. Er ist außerdem Teil Ihrer vorläufigen Domain. Eine eigene Domain können Sie später verbinden.'
})
